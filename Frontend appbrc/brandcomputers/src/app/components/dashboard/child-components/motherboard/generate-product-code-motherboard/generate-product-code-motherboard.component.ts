...
@Component({
  selector: 'app-generate-product-code-motherboard',
  templateUrl: './generate-product-code-motherboard.component.html',
  styleUrls: ['./generate-product-code-motherboard.component.scss']
})
export class GenerateProductCodeMotherboardComponent implements OnInit {
  ...
  getEventSearchResult(event: any) {
    ...
    this.router.navigate(['/dashboard/' + Motherboard.generateProductURL], {queryParams: {querySearch: this.searchQuery}})
    this.searchProductCodes();
  }

  clearSearch() {
    this.router.navigateByUrl('/dashboard/' + Motherboard.generateProductURL);
    this.getAllProductCodes();
  }
  ...
  public editProductCode(): void {
    this.generateProductCodeService.update(Motherboard.generateProductURL, this.id, this.validatingForm.value).subscribe(response => {
      this.router.navigate(['dashboard/' + Motherboard.generateProductURL]).finally(() => this.reloadPageService.reload());
  ...
  toggleForm(): void {
    ...
    if (closedForm) {
      this.router.navigate(['dashboard/' + Motherboard.generateProductURL]);
      this.isAddMode = true;
      this.validatingForm.reset();
    }
