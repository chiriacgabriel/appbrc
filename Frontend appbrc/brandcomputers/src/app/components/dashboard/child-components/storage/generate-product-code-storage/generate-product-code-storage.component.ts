...
@Component({
  selector: 'app-generate-product-code-storage',
  templateUrl: './generate-product-code-storage.component.html',
  styleUrls: ['./generate-product-code-storage.component.scss']
})
export class GenerateProductCodeStorageComponent implements OnInit {
  ...
  private getAllProductCodes(): void {
    this.generateProductCodeService.getAll(Storage.generateProductURL).subscribe((data: any) => {
  ...
  private searchProductCodes(): void {
    ...
    this.generateProductCodeService.search(Storage.generateProductURLSearch, this.params).subscribe((data: any) => {
  ...
  getEventSearchResult(event: any): void {
    ...
    this.router.navigate(['/dashboard/' + Storage.generateProductURL], {queryParams: {querySearch: this.searchQuery}});
    this.searchProductCodes();
  }

  clearSearch() {
    this.router.navigateByUrl('/dashboard/'  + Storage.generateProductURL);
    this.getAllProductCodes();
  }
  ...
  private addProductCode(): void {
    this.generateProductCodeService.add(Storage.generateProductURL, this.validatingForm.value).subscribe(response => {
  ...
  private editProductCode(): void {
    this.generateProductCodeService.update(Storage.generateProductURL , this.id, this.validatingForm.value).subscribe(response => {
      this.router.navigate(['dashboard/' + Storage.generateProductURL]).finally(() => this.reloadPageService.reload());
  ...
  deleteProductCode(generateProductCode: GenerateProductCode): void {
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          this.generateProductCodeService.delete(Storage.generateProductURL, Number(generateProductCode.id)).subscribe(response => {
            this.reloadPageService.reload();
          }, (error: HttpErrorResponse) => {
            this.errorMessage = error.error.message;
          });
  ...
  toggleForm(): void {
    ...
    if (closeForm) {
      this.router.navigate(['dashboard/' + Storage.generateProductURL]);
      this.isAddMode = true;
      this.validatingForm.reset();
    }
