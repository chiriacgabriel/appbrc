...
import { PowerSource } from "src/app/model/PowerSource";
import { GenerateProductCodeService } from "src/app/services/components/generate-product-code.service";
import { PowerSourceService } from "src/app/services/components/power-source.service";
...
export class PowerSourceComponent implements OnInit {
  ...
  powerSourceForm(): void {
    this.validatingForm = this.fromBuilder.group({
      generateProductCodePowerSource: new FormControl({value: '', disabled: true}, Validators.required),
  ...
  patchForm(powerSource) {
    this.isAddMode = false;
    const data = this.powerSourcesList.find(i => i.id == powerSource.id);

    this.selectedProductCode = data.generateProductCodePowerSource;
    this.validatingForm.patchValue(data);
    this.validatingForm.get('productName')
      .setValue(this.productCodeList
        .find(s => s.productCode === data.generateProductCodePowerSource.productCode));

    this.router.navigate(['dashboard/power-source'], {queryParams: {id: powerSource.id}});
  }
  ...
  getPowerSourcesByProductCode(productCode: string): void {
    this.router.navigate(['dashboard/power-source'], {queryParams: {productCode: productCode}});
  ...
  private addPowerSource(): void {
    ...
    this.formFields.generateProductCodePowerSource.enable();
    this.formFields.productName.disable();
    this.validatingForm.get('generateProductCodePowerSource').setValue(this.selectedProductCode);
  ...
  private editPowerSource() {
    ...
    this.powerSourceService.editById(this.id, this.validatingForm.value)
      .toPromise()
      .then((response) => {
        this.router.navigate(['dashboard/power-source']).finally(() => this.reloadPageService.reload());
        this.isAddMode = true;
        this.isSerialNumberPresent = false;
        document.querySelector('.modal-backdrop').remove();
  ...
  closeModal() {
    ...
    this.reloadPageService.skipLocation('dashboard/power-source');
