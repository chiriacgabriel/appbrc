...
import {VideoCardService} from '../../../../services/components/video-card.service';
...
export class VideoCardComponent implements OnInit {
  ...
  getVideoCardByProductCode(productCode: any) {
    ...
    this.router.navigate(['dashboard/video-card'], {queryParams: {productCode: productCode}});
  ...
  closeModal() {
    ...
    this.reloadPageService.skipLocation('dashboard/video-card');
  ...
  private editVideoCard(): void {
    ...
    this.videoCardService.editById(this.id, this.validatingForm.value)
      .toPromise()
      .then((response) => {
        this.router.navigate(['dashboard/video-card']).finally(() => this.reloadPageService.reload());
        this.isAddMode = true;
        this.isSerialNumberPresent = false;
        document.querySelector('.modal-backdrop').remove();
      }).catch((error: HttpErrorResponse) => {
  ...
  patchForm(videoCard: any) {
    this.isAddMode = false;
    const data = this.videoCardList.find(i => i.id == videoCard.id);
    this.router.navigate(['dashboard/video-card'], {queryParams: {id: videoCard.id}});
