...
@Component({
  selector: 'app-generate-product-code-fan-case',
  templateUrl: './generate-product-code-fan-case.component.html',
  styleUrls: ['./generate-product-code-fan-case.component.scss']
})
export class GenerateProductCodeFanCaseComponent implements OnInit {
  ...
  getEventSearchResult(event: any): void {
    ...
    this.router.navigate(['/dashboard/' + FanCase.generateProductURL], {queryParams: {querySearch: this.searchQuery}});
    this.searchProductCodes();
  }

  clearSearch() {
    this.router.navigateByUrl('/dashboard/' + FanCase.generateProductURL);
    this.getAllProductCodes();
  }
  ...
  private editProductCode(): void {
    this.generateProductCodeService.update(FanCase.generateProductURL, this.id, this.validatingForm.value).subscribe(response => {
      this.router.navigate(['dashboard/' + FanCase.generateProductURL]).finally(() => this.reloadPageService.reload());
  ...
  deleteProductCode(generateProductCode: GenerateProductCode): void {
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          this.generateProductCodeService.delete(FanCase.generateProductURL, Number(generateProductCode.id)).subscribe(response => {
            this.reloadPageService.reload();
          }, (error: HttpErrorResponse) => {
            this.errorMessage = error.error.message;
          });
  ...
  toggleForm(): void {
    ...
    if (closeForm) {
      this.router.navigate(['dashboard/' + FanCase.generateProductURL]);
      this.isAddMode = true;
      this.validatingForm.reset();
    }
