...
export class ProductCodeMemoryRamComponent implements OnInit {
  ...
  private productCodeForm(): void {
    this.validatingForm = this.formBuild.group({
      ...
      category: new FormControl('Memory RAM')
    });
  }
  ...
  public getAllProductCodes(): void {
    ...
    this.params = this.params.set('category', 'Memory RAM');
  ...
  private searchProductCodes(): void {
    this.params = this.params.set('query', this.searchQuery).set('categpry', 'Memory RAM');
