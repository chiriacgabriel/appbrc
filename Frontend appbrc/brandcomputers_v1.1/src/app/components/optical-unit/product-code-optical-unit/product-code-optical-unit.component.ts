...
@Component({
  ...
  templateUrl: './product-code-optical-unit.component.html',
  styleUrls: ['./product-code-optical-unit.component.css',
    '../../../../assets/css/_toggle_switch.css',
    '../../../../assets/css/_table_format.css']
})
export class ProductCodeOpticalUnitComponent implements OnInit {
  ...
  getEventSearchResult(event: any) {
    ...
    this.router.navigate(['/components/' + OpticalUnit.productCodeURI], {queryParams: {querySearch: this.searchQuery}})
    this.searchProductCodes();
  }

  clearSearch() {
    this.router.navigateByUrl('/components/' + OpticalUnit.productCodeURI);
    this.getAllProductCodes();
  }
  ...
  public editProductCode(): void {
    this.generateProductCodeService.update(OpticalUnit.generateProductURL, this.id, this.validatingForm.value).subscribe(response => {
      this.router.navigate(['components/' + OpticalUnit.productCodeURI]).then(() => this.reloadPageService.reload());
  ...
  deleteProductCode(productCode: GenerateProductCode): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.generateProductCodeService.delete(OpticalUnit.generateProductURL, Number(productCode.id)).subscribe(response => {
          this.reloadPageService.reload();
        }, (err: HttpErrorResponse) => {
          this.errorMessage = err.error.message;
        });
  ...
  toggleForm(): void {
    ...
    if (closedForm) {
      this.router.navigate(['components/' + OpticalUnit.productCodeURI]);
      this.isAddMode = true;
      this.validatingForm.reset();
    }
