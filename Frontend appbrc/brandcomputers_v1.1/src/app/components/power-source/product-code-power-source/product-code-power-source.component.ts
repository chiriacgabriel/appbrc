...
@Component({
  selector: 'app-product-code-power-source',
  templateUrl: './product-code-power-source.component.html',
  styleUrls: ['./product-code-power-source.component.css',
    '../../../../assets/css/_toggle_switch.css',
    '../../../../assets/css/_table_format.css']
})
export class ProductCodePowerSourceComponent implements OnInit {
  ...
  getEventSearchResult(event: any) {
    ...
    this.router.navigate(['/components/' + PowerSource.productCodeURI], {queryParams: {querySearch: this.searchQuery}})
    this.searchProductCodes();
  }

  clearSearch() {
    this.router.navigateByUrl('/components/' + PowerSource.productCodeURI);
    this.getAllProductCodes();
  }
  ...
  public editProductCode(): void {
    this.generateProductCodeService.update(PowerSource.generateProductURL, this.id, this.validatingForm.value).subscribe(response => {
      this.router.navigate(['components/' + PowerSource.productCodeURI]).then(() => this.reloadPageService.reload());
  ...
  toggleForm(): void {
    ...
    if (closedForm) {
      this.router.navigate(['components/' + PowerSource.productCodeURI]);
      this.isAddMode = true;
      this.validatingForm.reset();
    }
