...
export class MotherboardComponent implements OnInit {
  ...
  patchForm(motherboard: any, param: any) {
  ...
  closeModal() {
    ...
    this.reloadPageService.skipLocation('components/motherboard');
  ...
  private editMotherboard(): void {
    ...
    this.motherboardService.editById(this.id, this.validatingForm.value)
        .toPromise()
        .then((response) => {
          this.router.navigate(['components/motherboard']).then(() => this.reloadPageService.reload());
          this.isAddMode = true;
          this.isSerialNumberPresent = false;
          document.querySelector('.modal-backdrop').remove();
        }).catch((error: HttpErrorResponse) => {
